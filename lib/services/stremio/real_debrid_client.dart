...
class RealDebridClient {
  ...
  /// Add a torrent by infoHash (hex), building the magnet on the client.
  Future<String> addMagnetByHash(String infoHash) async {
    return addMagnet('magnet:?xt=urn:btih:$infoHash');
  }
  ...
  Future<String> resolveMagnetToDirectLink(
    ...
    // Prefer add-by-hash when the caller passed a magnet (same torrent, less
    // parsing on RD's side); falls back to the full magnet URI otherwise.
    final btih = RegExp(r'urn:btih:([a-zA-Z0-9]+)').firstMatch(magnetUri);
    final torrentId = btih != null
        ? await addMagnetByHash(btih.group(1)!)
        : await addMagnet(magnetUri);
    ...
    if (info['status'] == 'waiting_files_selection') {
      ...
      await selectFiles(torrentId, fileIds: best != null ? [best['id'].toString()] : null);
  ...
  /// Pick the best file for mobile playback: prefers 1080p (capped), then 720p,
  /// then falls back to the largest file under 8 GB. Skips files clearly exceeding
  /// 1080p-equivalent sizes (rough heuristic: >15 GB for a movie is likely 4K).
  static const _maxBytes = 8 * 1024 * 1024 * 1024; // 8 GB hard cap

  int _qualityScore(String filename) {
    final lower = filename.toLowerCase();
    // Exact resolution matches - higher score = better, capped at 1080p
    if (lower.contains('1080p') || lower.contains('1080i')) return 100;
    if (lower.contains('720p')) return 80;
    if (lower.contains('2160p') || lower.contains('4k')) return 0; // skip 4K
    if (lower.contains('480p')) return 40;
    // Heuristic: filenames with shorter durations / smaller sizes probably lower quality
    return 50; // unknown, middle priority
  }

  Map<String, dynamic>? _bestFile(List<Map<String, dynamic>> files) {
    ...
    for (final file in files) {
      ...
      final filename = file['path'] as String? ?? file['filename'] as String? ?? '';

      // Hard cap: skip files that are clearly 4K remuxes (>15 GB)
      if (bytes > 15 * 1024 * 1024 * 1024) continue;

      final score = _qualityScore(filename);

      // Prefer quality, then prefer smaller files at same quality tier
      if (score > bestScore || (score == bestScore && (bytes < bestBytes || bestBytes == 0))) {
        bestScore = score;
        bestBytes = bytes;
        best = file;
      }
    ...
    // Fallback: if quality scoring gave nothing useful, pick the largest
    // file under the hard cap
    if (best == null) {
      Map<String, dynamic>? fallback;
      var fallbackBytes = -1;
      for (final file in files) {
        final bytes = (file['bytes'] as num?)?.toInt() ?? 0;
        if (bytes > fallbackBytes && bytes <= _maxBytes) {
          fallbackBytes = bytes;
          fallback = file;
        }
      }
