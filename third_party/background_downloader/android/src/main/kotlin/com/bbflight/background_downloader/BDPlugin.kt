...
class BDPlugin : FlutterPlugin, MethodCallHandler, ActivityAware,
        ...
        suspend fun doEnqueue(
            ...
            if (useJobScheduler) {
                ...
                if (Build.VERSION.SDK_INT >= 34) {
                    // User-initiated data transfer job (UIDT). Without this flag the
                    // job is a plain background job and Android 14+ stops it seconds
                    // after start on battery pressure or constraint re-evaluation
                    // (observed: JobCancellationException ~4.6s after resume enqueue
                    // with batteryWhitelist:false). A UIDT job is exempt from
                    // background job quotas and constraint-re-eval stops. Requires the
                    // app to be in the foreground at schedule time — true at startup
                    // recovery; the try/catch on schedule() below falls back to a
                    // plain job when the app is not foreground (background auto-retry).
                ...
                val scheduleResult = try {
                    ...
                    // setUserInitiated requires the app in the foreground; fall back
                    // to a plain (non-UIDT) job rather than failing the enqueue.
                    Log.w(TAG, "UIDT schedule failed for ${task.taskId}: ${e.message}; retrying as plain job")
                    val plainBuilder = JobInfo.Builder(task.taskId.hashCode(), componentName)
                        .setRequiredNetworkType(if (taskRequiresWifi) JobInfo.NETWORK_TYPE_UNMETERED else JobInfo.NETWORK_TYPE_ANY)
                        .setRequiresCharging(false)
                        .setExtras(extras)
                    if (initialDelayMillis > 0) {
                        plainBuilder.setMinimumLatency(initialDelayMillis)
                    }
                    try {
                        jobScheduler.schedule(plainBuilder.build())
                    } catch (e2: Exception) {
                        Log.e(TAG, "JobScheduler schedule failed for ${task.taskId}: ${e2.message}")
                ...
                val constraints = Constraints.Builder().setRequiredNetworkType(
                    if (taskRequiresWifi) NetworkType.UNMETERED else NetworkType.NOT_REQUIRED
                ).build()
    ...
    private suspend fun methodEnqueue(call: MethodCall): Boolean =
        withContext(defaultScope.coroutineContext) {
            ...
            var isResume = args.size == 5
            var resumeData: ResumeData? = if (isResume) {
            ...
            // Auto-resume: a plain enqueue (no resume data) whose destination
            // already has a partial file should continue from it instead of
            // truncating. The partial name follows the taskId, so this only
            // matches when the enqueue reuses the original taskId (Plezy does).
            if (!isResume && resumeData == null) {
                try {
                    val destFilePath = task.filePath(applicationContext)
                    val partialPath = partialDownloadFilePath(destFilePath, task.taskId)
                    val partial = File(partialPath)
                    if (partial.exists() && partial.length() > 0L) {
                        resumeData = ResumeData(task, partialPath, partial.length(), null)
                        isResume = true
                        Log.i(TAG, "Auto-resuming task ${task.taskId} from partial ${partial.length()} bytes")
                    }
                } catch (e: Exception) {
                    Log.w(TAG, "Auto-resume check failed for ${task.taskId}: ${e.message}")
                }
            }
