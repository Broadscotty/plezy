...
class DownloadTaskRunner(context: TaskJobContext) : TaskRunner(context) {
    ...
    override suspend fun process(
        ...
        if (connection.responseCode in 200..206) {
            ...
            val storedETag = eTag
            if (isResume && storedETag != null && !storedETag.isEmpty() &&
                (eTagHeader != storedETag || storedETag.startsWith("W/"))
            ) {
                deleteTempFile()
    ...
    override fun determineIfResume(): Boolean {
        ...
        if (requiredStartByte == 0L) {
            // Disk fallback: a work auto-restarted after process death (e.g.
            // app force-killed while the worker was RUNNING) runs with its
            // ORIGINAL input, which predates the partial — so there is no
            // resume data even though a partial exists on disk. Adopt the
            // partial here; otherwise process() opens the output stream with
            // isResume=false and TRUNCATES it, restarting at 0%. Mirrors the
            // auto-resume patch in methodEnqueue (BDPlugin.kt).
            val destFilePath = task.filePath(context.appContext)
            ...
            if (partial.exists() && partial.length() > 0L) {
                requiredStartByte = partial.length()
                tempFilePath = partialPath
                Log.i(
                    TAG,
                    "Disk fallback: adopting partial ${partial.length()} bytes for task ${task.taskId}"
                )
                // NOTE: eTag stays null here (we have no stored eTag). The
                // server must return 206 for the resume to proceed; if it
                // returns 200, process() flips isResume=false and truncates
                // (unavoidable — server doesn't support resume).
            }
        }
        if (requiredStartByte == 0L) {
            return false
        ...
        // Fill tempFilePath from input resume data only when the disk
        // fallback above did not already set it. The input's stored filename
        // is authoritative when present (may be a SAF content:// URI); the
        // fallback path must not be overwritten by an empty input string.
        if (tempFilePath.isEmpty()) {
            tempFilePath = context.getInputString(keyResumeDataData) ?: ""
        }
