...
interface class FileDownloader {
  ...
  /// Removes the task with [taskId] from the WorkManager schedule without a
  /// status update and without deleting its temp file. Use this instead of
  /// [cancelTaskWithId] when a leftover worker must be cleared without
  /// destroying the partial it was writing.
  Future<void> killTaskWithId(String taskId) => _downloader.killTaskWithId(taskId);
  ...
  /// Store resume data for [resumeData.taskId] so a subsequent [resume]
  /// continues the download from the stored partial instead of restarting.
  ///
  /// Plezy uses this at startup to reconstruct resume state for downloads
  /// interrupted by an Android kill: without it the native side truncates
  /// the partial and starts from zero.
  Future<void> setResumeData(ResumeData resumeData) =>
      _downloader.setResumeData(resumeData);

  /// Retrieve stored resume data for [taskId], or null if none exists.
  Future<ResumeData?> getResumeData(String taskId) =>
      _downloader.getResumeData(taskId);
  ...
  /// destination-local partial files (`<dest>.<taskId hash>.part`, or
  /// `<dest>.part` from older builds). Active or resumable files
